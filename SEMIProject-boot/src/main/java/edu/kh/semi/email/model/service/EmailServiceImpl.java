...
public class EmailServiceImpl implements EmailService {
	...
	public String sendEmail(String htmlName, String email) {
		...
		// 2. DB 저장이 성공 된 경우에만 메일 발송 시도
		MimeMessage mimeMessage = mailSender.createMimeMessage(); // 메일 발송 시 사용하는 객체
		...
		try {
			...
			return authKey; // 모든 작업 성공 시 인증 키 반환
	...
	private String loadHtml(String authKey, String htmlName) {
	    ...
	    Context context = new Context(); // 템플릿에 바인딩 할 데이터를 담는 상자
	    context.setVariable("authKey", authKey); // 템플릿에서 사용할 변수 authKey 값

	    // templates/email/signup.html 형태로 템플릿 경로 작성
	...
	// 인증번호 발급 메서드
	// UUID를 사용하여 인증키 생성
	// (Unviersally unique IDentifier) : 전 세계에서 고유한 식별자를 생성하기 위한 표준
